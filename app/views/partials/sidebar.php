...
function toggleSidebar() {
	...
	if (sidebar.classList.contains('-translate-x-full')) {
		// Sidebar is closed - show menu button
		menuButton.classList.remove('hidden');
	} else {
		// Sidebar is open - hide menu button
...
document.addEventListener('click', function(event) {
	...
	if (!sidebar.contains(event.target) && !menuButton && window.innerWidth < 1024) {
		...
		// Show menu button when sidebar closes
...
window.addEventListener('resize', function() {
	...
	if (window.innerWidth >= 1024) {
		...
		document.getElementById('sidebarOverlay').classList.add('hidden');
		// Always show menu button on desktop (though it's hidden by lg:hidden)
		mobileMenuButton.classList.remove('hidden');
	}
});
</script>